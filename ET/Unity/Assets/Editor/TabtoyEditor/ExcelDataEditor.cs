...
{
    private const string path = "/Hotfix/TabToy/";
    ...
    public static void GenerateTabtoyILData()
    {
        string filePath = $"{Application.dataPath + "/Bundles/TabtoyData/tabtoyData.bytes"}";
        ParseExcelData parseExcelData = new ParseExcelData();
        Config config=parseExcelData.Init();
        //序列化写文件
        var il = new ILSerial();
        il.Write(filePath, config);
        AssetDatabase.Refresh();
        AssetDatabase.SaveAssets();
        ////读取
        //FileStream fs = null;
        //Config config1 = il.Read<Config>(filePath,out fs);
        //fs.Close();
        //var define = config1.GetAncestryByID(10101001);
        //Debug.LogError($"热更层：{define.Level}");
    }
    ...
    public static void UpdateSerializable()
    {
        ...
        GenerateCode(replaceStrings , "../../Hotfix/TabToy/GenerateClass/ConfigFixed.cs");
    }
