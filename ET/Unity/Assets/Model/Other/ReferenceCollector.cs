...
public class ReferenceCollector: MonoBehaviour, ISerializationCallbackReceiver
{
    ...
    public static void ClearAllObject(Dictionary<string, GameObject>  m_allLayersDic)
    {
        List<GameObject> dictUIGo = new List<GameObject>();
        if (m_allLayersDic.Count>0)
        {
            foreach (var eachKey in m_allLayersDic.Keys)
            {
                dictUIGo.Add(m_allLayersDic[eachKey]);
            }
            foreach (var willRemoveUIGo in dictUIGo)
            {
                Transform trans = willRemoveUIGo.transform;
