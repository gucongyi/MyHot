...
function IndexOfUsedNames(targetName)
{ 
	for(var i=0;i<arNamesUsed.length;i++)
	{ 
		if(targetName===arNamesUsed[i])
		{ 
			return i; 
		}
	} 
...
function exportScrollView(obj)
{
	...
	var isGridLayout = false;//grid类型的布局
    if (params.length > 2)
    {
        if(params.length != 4)
		{
	...
	var renderHorizontalGap;//水平间距
    var renderVerticalGap;//垂直间距
	if(isGridLayout)
	{
		renderHorizontalGap = params[3] > 1 ? (recSize.width - rec0.width * params[3])/(params[3] - 1) : 0;//(size - 每个格子宽度*多少个格子)/(多少列-1)
		renderVerticalGap = params[2] > 1 ? (recSize.height - rec0.height * params[2])/(params[2] - 1) : 0;
	}
	...
	if(isGridLayout)
	{
		sceneData += "<string>" + parseInt(params[2]) + "</string>";   //行数
		sceneData += "<string>" + parseInt(params[3]) + "</string>";   //列数
...
function makeValidFileName(fileName)
{
    ...
    {
		var nameUsedIndex = IndexOfUsedNames(validName);
		if( nameUsedIndex!=-1 )
		{
			validName += "_" + uuid++;
		}
